...
- **Only agents that sit on the genuine capability frontier go in `CURATED_PUBLIC_BUILTIN_AGENT_IDS`.** The bar is stricter than "real tool" or even "platform primitive" — the agent must do something a free frontier agent (Claude Code / OpenClaw / Hermes) cannot simply do for itself. Current count: **3 curated public agents** (`dependency_auditor`, `lighthouse_auditor`, `accessibility_auditor`) after the **2026-06-21 frontier-evidence cull**, which sunset the other 8 (cve_lookup, dns_inspector, python_executor, multi_language_executor, live_sandbox, db_sandbox, browser_agent, site_navigator) on the evidence of `experiments/builtin-frontier` (free harnesses hit ~86% built-in success and route around missing tools) and `experiments/deference` (push-routing commodity tasks was net-negative). `SUNSET_DEPRECATED_AGENT_IDS` now holds **37 entries**. Internal endpoints stay wired so old job IDs and signed receipts continue to resolve, but a NEW call to a sunset agent returns a clean HTTP 410 `agent.sunset` from `_assert_agent_callable` (`part_002.py`) — sunset means removed from the catalog AND no longer callable, not merely hidden. Do not add LLM-only agents, and do not re-add commodity agents a free harness handles.
...
Source of truth: `server/builtin_agents/constants.py`. Curated public set (agents on the genuine capability frontier — work a free frontier agent can't simply do itself) is in `CURATED_PUBLIC_BUILTIN_AGENT_IDS` — currently **3 agents** after the 2026-06-21 frontier-evidence cull: `dependency_auditor` (structural multi-step audit win), `lighthouse_auditor`, and `accessibility_auditor` (verification frontier — built-ins fabricate the measurement). The 2026-06-21 cull sunset the other 8 (cve_lookup, dns_inspector, python_executor, multi_language_executor, live_sandbox, db_sandbox, browser_agent, site_navigator) on the evidence in `experiments/builtin-frontier` and `experiments/deference`. See `SUNSET_DEPRECATED_AGENT_IDS` for the 37 sunset entries and per-agent reasoning. Internal/hidden agents are in the same file. Always read constants directly; do not duplicate IDs anywhere else.
