"""
exceptions.py — Typed exceptions for the Aztea SDK.

All SDK methods raise subclasses of AzteaError rather than returning
...
class AzteaError(Exception):
    """Base class for all Aztea SDK errors."""
...
class InsufficientFundsError(AzteaError):
...
class AgentNotFoundError(AzteaError):
...
class RateLimitError(AzteaError):
...
class AuthenticationError(AzteaError):
...
class PermissionError(AzteaError):
...
class ClarificationNeededError(AzteaError):
    """
    Raised by ``AzteaClient.hire()`` when the agent paused and needs
