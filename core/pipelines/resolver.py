...
def _resolve_value(value: Any, pipeline_input: dict, step_results: dict[str, Any]):
    if isinstance(value, str):
        ...
        if value.startswith("$input."):
            return _lookup_path(
                pipeline_input,
                [part for part in value[len("$input.") :].split(".") if part],
                value,
            )
        if value.startswith("$"):
            ...
            if len(parts) >= 2 and parts[1] == "output":
                ...
                if node_id not in step_results:
                    raise ValueError(
                        f"Could not resolve '{value}'. Step '{node_id}' has not completed."
                    )
    ...
    if isinstance(value, dict):
        return {
            str(key): _resolve_value(item, pipeline_input, step_results)
...
def resolve_input_map(
    input_map: dict, pipeline_input: dict, step_results: dict[str, Any]
) -> dict:
