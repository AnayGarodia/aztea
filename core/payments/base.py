...
def post_call_refund_difference(
    ...
    *,
    ...
    """Insert a compensating refund for variable-pricing overestimates.

    Called after a successful call completes and the actual usage turns
    out cheaper than the pre-charge estimate. Writes one new ledger row
    (insert-only) linked to ``charge_tx_id`` and credits the caller's
    wallet cache in the same SQL transaction, preserving the ledger
    invariant. Returns the new tx_id, or ``None`` if nothing was
    refunded (e.g. a prior refund for the same charge is already on
    record).

    Integer cents only. Never subtracts from the original charge row —
    that row stays untouched; the new entry is the audit-visible
    correction.
    """
    ...
    if amount == 0:
        return None
    normalized_memo = str(memo or "").strip() or (
        f"Variable-pricing refund for call {charge_tx_id[:8]}"
    )
    with _conn() as conn:
        conn.execute("BEGIN IMMEDIATE")
        ...
        try:
            ...
            # Idempotent: a refund for this (charge, wallet, type) is
            # already on the ledger. Treat as already-applied.
            return None
