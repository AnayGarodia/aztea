...
def _decision_cache_key(
    ...
    # caller_owner_id is part of the key because Phase 1 (C3) per-caller
    # affinity bias changes the chosen agent per-caller for the same
    # intent. Caching by intent alone would serve caller A's biased
    # winner to caller B. caller_owner_id=None (anonymous) shares one
    # bucket — correct, since no per-caller bias applies.
    return (
...
def decide_cached(
    *,
    ...
    Cache key = (intent_hash, max_cost_usd_cents, aggressive,
    catalog_version, caller_owner_id). Per-caller bias (Phase 1 C3) makes
    the chosen agent caller-specific for the same intent, so the key
    includes the caller. The same intent against a mutated catalog
    (bumped version) misses the cache, so any registry mutation
    invalidates all entries atomically.
    ...
    ``request_budget`` is forwarded to ``decide`` for the per-request LLM
    cap — never participates in the cache key because cache hits don't
    consume budget.
    """
    try:
    ...
    cache_key = _decision_cache_key(
        intent, max_cost_usd, aggressive, version_at_start, caller_owner_id
    )
