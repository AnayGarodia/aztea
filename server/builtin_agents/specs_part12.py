    ...
    return [
        {
            ...
            "name": "Site Navigator",
            "description": (
                "Use when you need structured data from a live web page and you "
                "have a goal, not a scripted set of clicks. Give it a URL and a "
                "plain-English goal (e.g. 'list the pricing tiers and what each "
                "includes'); it renders the page with headless Chromium, reads "
                "the accessibility tree (far cheaper than a screenshot), and "
                "returns the structured answer plus a reusable site map of the "
                "page's links, buttons, inputs, and headings. Can also return clean "
                "LLM-ready markdown, raw HTML, or a links list via the 'formats' "
                "option (goal optional for those). For static/SSR pages and sites with "
                "a discoverable JSON API it skips the browser entirely. Differs from the "
                "Browser Agent (which renders/screenshots a page you drive) by "
                "resolving a goal into data. If no LLM provider is configured it "
                "degrades to returning the retrieved data."
            ),
