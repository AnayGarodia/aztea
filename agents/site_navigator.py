...
def _attach_observation_receipt(
    ...
    a signing/persistence failure never breaks the navigation. ``http_status`` /
    ``content_type`` are recorded when the serving path knew them (http_first); the
    Chromium path leaves them None rather than guessing a status it never saw.
    """
    try:
...
_WRITE_ACTIONS = ("interact", "preview", "dry_run", "commit")


def run(payload: dict[str, Any]) -> dict[str, Any]:
    """The unified Web Agent: READ by default, ACT when asked.

    With no ``action`` (or a read call) it navigates ``url`` and resolves ``goal`` over
    the cheapest path that works — a signed API-spec replay (no browser) → an HTTP-first
    static fetch (no browser) → the full Chromium render (both no-browser paths flag-
    gated and fail-closed, so with AZTEA_API_DISCOVERY and AZTEA_HTTP_FIRST off this is
    exactly the Chromium read path, output-equivalent to before).

    When ``action`` is interact / preview / dry_run / commit it delegates to the
    web_actor engine, which is itself fail-closed: every write path is OFF until
    AZTEA_ACTION_WEB_ENABLED=1 (commit + credentials need their own flags). So a default
    deploy of this one agent still only reads.
    """
    if isinstance(payload, dict) and str(payload.get("action") or "").strip().lower() in _WRITE_ACTIONS:
        # Lazy import: keep the write/credential engine off the read path's import graph
        # until an action is actually requested.
        from agents import web_actor
        return web_actor.run(payload)
...
def _try_http_first(
    *, url: str, goal: str, formats: tuple[str, ...], schema: dict[str, Any] | None, t_start: float,
    ...
    # The static fetch is the one path that actually saw the HTTP exchange — surface
    # its status so the public API's metadata.statusCode isn't a hardcoded 200.
    ...
    if feature_flags.observation_receipts_enabled():
        _attach_observation_receipt(
            result, requested_url=url, final_url=final_url, rows=analysis.rows,
