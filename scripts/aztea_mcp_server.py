...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--base-url",
        default=os.environ.get("AZTEA_BASE_URL", "http://localhost:8000"),
        help="Aztea HTTP base URL (default: AZTEA_BASE_URL or http://localhost:8000).",
    )
    parser.add_argument(
        "--api-key",
        default=os.environ.get("AZTEA_API_KEY", ""),
        help="Caller API key (default: AZTEA_API_KEY).",
    )
    parser.add_argument(
        "--refresh-seconds",
        type=int,
        default=int(os.environ.get("AZTEA_MCP_REFRESH_SECONDS", "60")),
        help="Tool manifest refresh interval in seconds (default: 60).",
    )
    parser.add_argument(
        "--timeout-seconds",
        type=float,
        default=float(os.environ.get("AZTEA_MCP_TIMEOUT_SECONDS", "10")),
...
def main() -> None:
    ...
    if not api_key:
        _LOG.warning(
            ...
            "tool calls will return a sign-up link. Set AZTEA_API_KEY=am_... to enable full access."
        )
