"""test_agent_bisect_and_blame.py — A2 Bisect & Blame (~10 tests)."""
from __future__ import annotations

import os

import pytest

os.environ.setdefault("API_KEY", "test-master-key")
os.environ.setdefault("SERVER_BASE_URL", "http://localhost:8000")

from core.llm.errors import BudgetExceededError, LLMError
from tests.agent_helpers import (
    _capture_llm_calls,
    _make_response,
    _stub_llm_factory,
    assert_error_envelope,
    assert_reasoning_loop,
    patch_llm_everywhere,
    set_env_for,
)

from agents import bisect_and_blame


_VALID_PAYLOAD = {
    "good_ref": "abc",
    "bad_ref": "def",
    "repro_cmd": "pytest tests/x.py",
}


def _clear_runner_env(monkeypatch):
    monkeypatch.delenv("AZTEA_RUNNER_JOB_LIFECYCLE_ENABLED", raising=False)


def test_invalid_input_envelope():
    out = bisect_and_blame.run("not a dict")  # type: ignore[arg-type]
    assert_error_envelope(out, "bisect_and_blame.invalid_input")


def test_missing_good_ref_rejected(monkeypatch):
    _clear_runner_env(monkeypatch)
    out = bisect_and_blame.run({"bad_ref": "def", "repro_cmd": "pytest"})
    assert_error_envelope(out, "bisect_and_blame.invalid_input")


def test_missing_bad_ref_rejected(monkeypatch):
    _clear_runner_env(monkeypatch)
    out = bisect_and_blame.run({"good_ref": "abc", "repro_cmd": "pytest"})
    assert_error_envelope(out, "bisect_and_blame.invalid_input")


def test_missing_repro_cmd_rejected(monkeypatch):
    _clear_runner_env(monkeypatch)
    out = bisect_and_blame.run({"good_ref": "abc", "bad_ref": "def"})
    assert_error_envelope(out, "bisect_and_blame.invalid_input")


def test_long_ref_strings_accepted(monkeypatch):
    """40-char SHAs are accepted by input validation."""
    _clear_runner_env(monkeypatch)
    good = "a" * 40
    bad = "b" * 40
    out = bisect_and_blame.run({
        "good_ref": good, "bad_ref": bad, "repro_cmd": "pytest tests/x.py",
    })
    # No runner env → got past input validation into config gate.
    err = assert_error_envelope(out, "bisect_and_blame.requires_configuration")
    assert err["details"]["good_ref"] == good


def test_requires_configuration_when_runner_disabled(monkeypatch):
    _clear_runner_env(monkeypatch)
    out = bisect_and_blame.run(_VALID_PAYLOAD)
    err = assert_error_envelope(out, "bisect_and_blame.requires_configuration")
    missing_blob = " ".join(err["details"]["missing"])
    assert "AZTEA_RUNNER_JOB_LIFECYCLE_ENABLED" in missing_blob


def test_happy_path_reaches_reasoning_loop(monkeypatch):
    set_env_for("bisect_configured", monkeypatch)
    patch_llm_everywhere(monkeypatch, _stub_llm_factory(
        '{"blamed_commit": "deadbeef", "rationale": "x"}',
    ))
    out = bisect_and_blame.run(_VALID_PAYLOAD)
    assert "error" not in out, f"unexpected error envelope: {out!r}"
    assert "plan" in out and "synthesis" in out and "trace" in out
    assert_reasoning_loop(out)


def test_reasoning_loop_two_calls(monkeypatch):
    set_env_for("bisect_configured", monkeypatch)
    stub, calls = _capture_llm_calls()
    patch_llm_everywhere(monkeypatch, stub)
    bisect_and_blame.run(_VALID_PAYLOAD)
    assert len(calls) >= 2, f"expected >= 2 LLM calls, got {len(calls)}"


def test_budget_exceeded_returns_envelope(monkeypatch):
    set_env_for("bisect_configured", monkeypatch)

    def _boom(req, *args, **kwargs):
        raise BudgetExceededError(
            "stub", "stub-model", "budget exhausted",
            budget_cents=1, spent_cents=0, estimated_next_cents=5,
        )
    patch_llm_everywhere(monkeypatch, _boom)
    out = bisect_and_blame.run(_VALID_PAYLOAD)
    assert_error_envelope(out, "bisect_and_blame.llm_error")


def test_llm_error_propagates(monkeypatch):
    set_env_for("bisect_configured", monkeypatch)

    def _boom(req, *args, **kwargs):
        raise LLMError("stub", "stub-model", "upstream blew up")
    patch_llm_everywhere(monkeypatch, _boom)
    out = bisect_and_blame.run(_VALID_PAYLOAD)
    assert_error_envelope(out, "bisect_and_blame.llm_error")
